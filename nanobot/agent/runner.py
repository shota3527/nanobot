...
class AgentRunner:
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        for iteration in range(spec.max_iterations):
            ...
            response = hook.prepare_response(context, response)
            ...
            if response.should_execute_tools:
                ...
                finalized_results = [
                    hook.finalize_tool_result(context, tool_call, result)
                    for tool_call, result in zip(response.tool_calls, results)
                ]
                context.tool_results = list(finalized_results)
                for tool_call, result in zip(response.tool_calls, finalized_results):
                    tool_message = {
                        "role": "tool",
